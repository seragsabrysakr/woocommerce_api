...
extension WooOrderApi on FlutterWooCommerce {
  ...
  Future<List<WooOrder>> getOrders({
    ...
    try {
      ...
      if (response.statusCode != null &&
        ...
        return res.map((item) => WooOrder.fromJson(item)).toList();
      } else {
  ...
  Future<WooOrder> getOrder({required int id, bool? useFaker}) async {
    final isUsingFaker = useFaker ?? this.useFaker;
